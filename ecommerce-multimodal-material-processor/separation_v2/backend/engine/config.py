...
class Settings(BaseSettings):
    ...
    #     比原硬编码段更详细准确（含视频专项、易错红线、正例），single_display 定义严格。
    ...
    #     本文件在 engine/，代码用文档在其上一级 backend/（docs、references），故为 ".."。
    #     线上部署把 backend/ 上传到服务器根并在其下运行，config.py 位于 <root>/engine/，
    #     文档位于 <root>/docs、<root>/references，".." 同样命中，无需按环境改动。
...
def resolve_template_path(name: str = None) -> Path:
    ...
    模板均为本地 ./docs/* 文件，路径相对 *backend 根目录* 解析（见 _resolve_repo_file）。
    """
...
def resolve_grounding_path(rel_path: str = None) -> Path:
    ...
    文档解析为绝对路径，供 labeler 注入分类 prompt。文档实际位于 backend 内。
    """
