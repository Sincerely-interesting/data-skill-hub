...
class MaterialLabeler:
    ...
    def __init__(
        ...
        # 记录 CLI 显式传入的 --model（区别于 .env 默认值），用于修复模型优先级 bug。
    ...
    def _load_provider_config(self) -> None:
        ...
        # 2) 模型优先级（BUG 修复）：CLI --model > .env 的 VLM_MODEL > provider 表硬编码。
        #    旧逻辑 `config["model"] or vlm_model` 会让 provider 表的硬编码模型覆盖 .env：
        #    例如 --provider minicpm 用 "minicpm-v-4" 覆盖 .env 的 "MiniCPM-V-4.5-9B"，
        #    导致面壁托管端点(api.modelbest.cn)回 404 model_not_found；且 --model 被忽略。
        self.model = self._cli_model or settings.vlm_model or config["model"]
    ...
    async def label_single_folder(self, folder_path: Path) -> Dict:
        ...
        try:
            ...
            if ok:
                ...
                # 优先从 reasoning 反提最终标签：A+B 长文档注入后模型常在
                # label 字段填错（如统一'其他'），但 reasoning 判定路径结论正确。
